...
def auth_user(login, password) -> User:
    res = db.execute_query('select * from users where login = ? and password = ?', login, password, is_select=True)['result'][0]
    print(res)
    return create_cls_object(res, User)
...
def get_user_for_login(login) -> User:
    res = db.execute_query('select * from users where login = ?', login, is_select=True)['result'][0]
    return create_cls_object(res, User)

def get_user_for_id(user_id) -> User:
    res = db.execute_query('select * from users where id = ?', user_id,  is_select=True)['result'][0]
    return create_cls_object(res, User)
